...
export default function TeacherAnnouncementFeed() {
  ...
  if (announcements.length === 0) {
    return (
      <div className="mx-auto max-w-4xl space-y-6 p-6">
        <div className="rounded-2xl bg-linear-to-br from-[#0F172A] via-[#111C33] to-[#1E3A8A] p-6 sm:p-8">
          <h1 className="text-2xl font-semibold tracking-tight text-white">School Announcements</h1>
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-linear-to-br from-[#0F172A] via-[#111C33] to-[#1E3A8A] px-6 py-7 sm:px-8">
        <div className="absolute -right-16 -top-24 h-56 w-56 rounded-full bg-[#2563EB]/15 blur-3xl" />
        ...
        <div className="relative">
          ...
          <h1 className="text-2xl font-semibold tracking-tight text-white sm:text-3xl">School Announcements</h1>
      ...
      <div className="space-y-4">
        {announcements.map((a) => {
          ...
          return (
            <div
              key={a.id}
              className={`overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-150 hover:-translate-y-0.5 hover:shadow-md ${badge.borderClass}`}
            >
              <div className="p-5 sm:p-6">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="whitespace-nowrap text-xs text-slate-400">{timeAgo(a.publishDate)}</span>
                </div>
                ...
                <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#1E3A8A] text-xs font-semibold text-white">
                      {initials(a.postedBy.name)}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-slate-900">{a.postedBy.name}</p>
                      <p className="text-xs uppercase tracking-wide text-slate-400">{posterLabel}</p>
                    </div>
                  </div>
